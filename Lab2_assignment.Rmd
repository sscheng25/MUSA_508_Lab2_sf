...
ggplot()+
  geom_sf(data = acsTractsPHL.2019.sf, aes(fill = total_pop.2019),
          color = "transparent")+
  geom_sf(data = acsTractsPHL.2019.sf %>%
            filter(Neighborhood == "My Neighborhood") %>%
            st_union(),
          color = "white",
          fill = "transparent")+
  # Bonus to figure out different color ramps with scale_fill_viridis()
  labs(
    title = "Population of Philadelphia",
    subtitle = "Year: 2019",
    caption = "Data: US Census Bureau, ACS")
